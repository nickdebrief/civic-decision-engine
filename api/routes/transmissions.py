...
def _render_index_rows(rows: list[dict]) -> str:
    ...
    for item in rows:
        ...
        rendered.append(
            f"""<tr>
              <td class="transmission-reference table-cell--identifier"><a href="/transmissions/{escape(reference)}">{escape(reference)}</a></td>
              <td class="transmission-title table-cell--content"><strong>{escape(_display(item.get('title')))}</strong><p>{escape(_display(item.get('summary')))}</p></td>
              <td class="transmission-sender table-cell--content">{escape(_display(item.get('sender')))}</td>
              <td class="transmission-recipient table-cell--content">{escape(_display(item.get('recipient')))}</td>
              <td class="transmission-method table-cell--label">{escape(_display(item.get('communication_method_label')))}</td>
              <td class="transmission-publication table-cell--compact">{escape(_date(item.get('published_at') or item.get('transmission_date')))}</td>
              <td class="transmission-count table-cell--compact">{int(item.get('attached_object_count') or 0)}</td>
              <td class="transmission-actions table-cell--action"><a href="/transmissions/{escape(reference)}">Open Transmission</a></td>
            </tr>"""
        )
    return "".join(rendered)
...
def public_transmission_library(
    ...
    return HTMLResponse(content=f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Public Transmission Library | Civic Decision Engine</title><link rel="canonical" href="/transmissions"><meta name="description" content="Public index of governed communication-context transmissions in the Civic Decision Engine."><style>*{{box-sizing:border-box}}body{{margin:0;background:#f7f7f4;color:#1f2933;font-family:system-ui,sans-serif}}main{{width:min(1180px,calc(100% - 32px));margin:32px auto 64px}}h1,h2{{color:#143a52}}a{{color:#245d61}}{PUBLIC_NAVIGATION_CSS}{PUBLIC_GOVERNANCE_TABLE_CSS}.transmission-boundary,.transmission-summary{{padding:14px 16px;border-left:4px solid #2e8b9a;background:#fff;line-height:1.55}}.transmission-summary{{margin:18px 0;border-left-color:#143a52}}.transmission-filters{{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;background:#fff;border:1px solid #d8d4ca;padding:16px;margin:22px 0}}.transmission-filters label{{display:grid;gap:6px;color:#555;font:.78rem ui-monospace,monospace;text-transform:uppercase}}.transmission-filters input,.transmission-filters select{{width:100%;padding:9px;border:1px solid #c9c6bd;background:#fff;font:.92rem system-ui,sans-serif}}.transmission-filters button,.transmission-filters a{{width:max-content;padding:9px 12px;border:0;background:#245d61;color:#fff;cursor:pointer;text-decoration:none;display:inline-block}}.transmission-table{{min-width:1160px;font-size:.9rem}}.transmission-reference{{min-width:150px}}.transmission-title{{min-width:300px;width:34%}}.transmission-title p{{margin:.35rem 0 0;color:#555;line-height:1.45}}.transmission-sender,.transmission-recipient{{min-width:170px}}.transmission-method{{min-width:120px}}.transmission-publication{{min-width:130px}}.transmission-count{{min-width:120px}}.transmission-actions{{min-width:140px}}.transmission-empty{{text-align:center;color:#555;padding:18px}}.transmission-pagination{{display:flex;gap:16px;align-items:center;flex-wrap:wrap;margin:18px 0}}@media(max-width:760px){{.transmission-filters{{grid-template-columns:1fr}}.transmission-table{{min-width:1060px}}}}</style></head><body><main>{public_primary_navigation(active="transmissions")}{public_breadcrumbs([("Home", "/"), ("Archive", "/archive"), ("Transmissions", None)])}<h1>Public Transmission Library</h1><p>{object_type_badge("public_transmission")}</p><p class="transmission-boundary">{escape(BOUNDARY_TEXT)} This library shows only Published transmissions that are public.</p><section class="transmission-summary"><p><strong>Total matching Public Transmissions:</strong> {int(result['total'])}</p><p><strong>Active filters:</strong> {escape(_active_filter_summary(filters))}</p><p><strong>Page:</strong> {int(result['page'])} of {int(result['page_count'])}</p></section><form class="transmission-filters" method="get" action="/transmissions"><label>Search<input name="q" value="{escape(str(filters['q']))}" placeholder="Reference, sender, recipient, subject, title, or included object reference"></label><label>Sender<select name="sender">{_option_list(options['senders'], str(filters['sender']), 'Any sender')}</select></label><label>Recipient<select name="recipient">{_option_list(options['recipients'], str(filters['recipient']), 'Any recipient')}</select></label><label>Communication method<select name="method">{_option_list(method_options, str(filters['method']), 'Any method')}</select></label><label>Publication year<select name="year">{_option_list(options['years'], str(filters['year']), 'Any year')}</select></label><label>Page size<select name="page_size">{_page_size_options(int(result['page_size']))}</select></label><button type="submit">Apply filters</button><a href="/transmissions">Clear filters</a></form><div class="transmission-pagination"><span>Page {int(result['page'])} of {int(result['page_count'])}</span>{previous_link}{next_link}</div><div class="transmission-results governance-table-wrap" role="region" aria-label="Public Transmission Library table"><table class="transmission-table governance-table governance-table--dense"><thead><tr><th scope="col" class="transmission-reference table-cell--identifier">Reference</th><th scope="col" class="transmission-title table-cell--content">Title and summary</th><th scope="col" class="transmission-sender table-cell--content">Sender</th><th scope="col" class="transmission-recipient table-cell--content">Recipient</th><th scope="col" class="transmission-method table-cell--label">Method</th><th scope="col" class="transmission-publication table-cell--compact">Publication</th><th scope="col" class="transmission-count table-cell--compact">Included governed objects</th><th scope="col" class="transmission-actions table-cell--action">Actions</th></tr></thead><tbody>{body_rows}</tbody></table></div><div class="transmission-pagination"><span>Page {int(result['page'])} of {int(result['page_count'])}</span>{previous_link}{next_link}</div></main></body></html>""")


def _render_attachments(attachments: list[dict]) -> str:
    rows = "".join(
        f"""<tr>
          <td class="transmission-attachment-position table-cell--compact">{escape(_display(item.get('position')))}</td>
          <td class="transmission-attachment-reference table-cell--identifier">{escape(_display(item.get('attachment_reference')))}</td>
          <td class="transmission-attachment-type table-cell--label">{object_type_badge(str(item.get('object_type') or ''))}</td>
          <td class="transmission-attachment-object table-cell--content"><a href="{escape(str(item.get('object_url') or '#'))}">{escape(_display(item.get('object_title')))}</a>{f'<p>{escape(str(item.get("object_summary") or ""))}</p>' if item.get('object_summary') else ''}</td>
          <td class="transmission-attachment-object-reference table-cell--identifier">{escape(_display(item.get('object_public_reference')))}{f'<br><span class="table-cell--content">Optional Reference Identifier: {escape(str(item.get("object_secondary_reference") or ""))}</span>' if item.get('object_secondary_reference') else ''}</td>
          <td class="transmission-attachment-relationship table-cell--label">{escape(_display(item.get('relationship_label')))}</td>
          <td class="transmission-attachment-note table-cell--note">{escape(_display(item.get('public_note')))}</td>
        </tr>"""
    ...
    return f"""<section><h2>Included Governed Objects</h2><p class="transmission-boundary">Each referenced object keeps its own identity, lifecycle, provenance, verification, and public page. The Transmission preserves communication context only.</p><div class="transmission-results transmission-attachments-wrapper governance-table-wrap" role="region" aria-label="Included Governed Objects table"><table class="transmission-table transmission-attachments-table governance-table governance-table--dense"><colgroup><col class="transmission-attachment-position-col"><col class="transmission-attachment-reference-col"><col class="transmission-attachment-type-col"><col class="transmission-attachment-object-col"><col class="transmission-attachment-object-reference-col"><col class="transmission-attachment-relationship-col"><col class="transmission-attachment-note-col"></colgroup><thead><tr><th scope="col" class="table-cell--compact">Position</th><th scope="col" class="table-cell--identifier">Inclusion reference</th><th scope="col" class="table-cell--label">Object type</th><th scope="col" class="table-cell--content">Object</th><th scope="col" class="table-cell--identifier">Object reference</th><th scope="col" class="table-cell--label">Relationship</th><th scope="col" class="table-cell--note">Public note</th></tr></thead><tbody>{rows}</tbody></table></div></section>"""
...
def _render_pathway(events: list[dict]) -> str:
    ...
    return f'<section><h2>Transmission Publication Provenance</h2><div class="governance-table-wrap" role="region" aria-label="Transmission Publication Provenance table"><table class="metadata governance-table governance-table--dense transmission-provenance-table"><thead><tr><th scope="col" class="table-cell--timestamp">Timestamp</th><th scope="col" class="table-cell--action">Action</th><th scope="col" class="table-cell--compact">Actor</th><th scope="col" class="table-cell--note">Note</th></tr></thead><tbody>{rows}</tbody></table></div></section>'
