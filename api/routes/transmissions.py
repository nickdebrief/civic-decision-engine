...
def _render_attachments(attachments: list[dict]) -> str:
    ...
    if not rows:
        rows = '<tr><td colspan="7" class="transmission-empty">No attached governed public objects are visible for this Transmission.</td></tr>'
    return f"""<section><h2>Attached Governed Objects</h2><p class="transmission-boundary">Each transmitted object keeps its own identity, lifecycle, provenance, verification, and public page. The Transmission preserves communication context only.</p><div class="transmission-results"><table class="transmission-table"><thead><tr><th>Position</th><th>Attachment reference</th><th>Object type</th><th>Object</th><th>Object reference</th><th>Relationship</th><th>Public note</th></tr></thead><tbody>{rows}</tbody></table></div></section>"""
...
def public_transmission_page(transmission_reference: str, return_to: str | None = None):
    ...
    fields = (
        ...
        ("Attached governed objects", len(attachments)),
    )
