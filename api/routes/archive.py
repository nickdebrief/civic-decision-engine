...
def _media_filter_value(document: dict[str, Any]) -> str:
    ...
    if family == "document" and document_type_label(document.get("document_type")) == "PDF":
        return "pdf"
    if family in {"image", "audio", "spreadsheet", "rich_text", "email", "mailbox"}:
        return family
    return ""
...
def _collection_member_keys(conn: sqlite3.Connection, collection_reference: str) -> set[tuple[str, str]]:
    ...
    for member in acm.list_public_collection_memberships(conn, collection["id"], root=intake_root()):
        ...
        if object_type:
            for candidate in (
                member.get("member_public_reference"),
                member.get("member_reference"),
                member.get("document_identifier"),
                member.get("document_reference"),
            ):
                member_reference = str(candidate or "").strip()
                if member_reference:
...
def _outlook_archive_payload(item: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "preservation_complete": metadata.get("preservation_complete", False),
        "preservation_timestamp": metadata.get("preservation_timestamp"),
        "preservation_completed_at": metadata.get("preservation_completed_at"),
        "hash_verification_status": metadata.get("hash_verification_status"),
        "inspection_complete": metadata.get("inspection_complete", False),
        "inspection_timestamp": metadata.get("inspection_timestamp"),
        "archive_health": metadata.get("archive_health"),
        "latest_archive_job_id": metadata.get("latest_archive_job_id"),
        "projection_state": metadata.get("projection_state"),
        "folder_projection_performed": metadata.get("folder_projection_performed", False),
        "message_projection_performed": metadata.get("message_projection_performed", False),
...
def public_outlook_archive_status(document_id: str):
    ...
    return {
        ...
        "preservation_complete": payload["preservation_complete"],
        "hash_verification_status": payload["hash_verification_status"],
        "inspection_complete": payload["inspection_complete"],
        "inspection_timestamp": payload["inspection_timestamp"],
        "projection_state": payload["projection_state"],
        "folder_projection_performed": payload["folder_projection_performed"],
        "message_projection_performed": payload["message_projection_performed"],
