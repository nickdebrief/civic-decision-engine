...
def _render_result_cards(rows: list[ArchiveResult]) -> str:
    if not rows:
    ...
    for item in rows:
        ...
        cards.append(
            f"""<article class="archive-result-card">
              ...
              <p class="archive-object-type">{escape(object_label)}</p>
              <p>{escape(item.summary or 'Governed public object with an independent public page.')}</p>
              <dl>{metadata_html}</dl>
              <p class="archive-actions"><a href="{escape(item.url)}">Open governed object</a></p>
            </article>"""
        )
    return "".join(cards)
