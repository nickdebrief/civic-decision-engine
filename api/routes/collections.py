...
def _render_collection_members(memberships: list[dict]) -> str:
    rows = "".join(
        f"""<tr>
          ...
          <td class="public-collection-member-document"><a href="/documents/{escape(str(item.get('document_id') or ''))}">{escape(_display(item.get('document_title')))}</a></td>
          <td class="public-collection-member-document-reference">{escape(_display(item.get('document_reference')))}</td>
          <td class="public-collection-member-format">{escape(_display(item.get('document_format')))}</td>
          <td class="public-collection-member-status">{escape(_display(item.get('document_status_label')))}</td>
          <td class="public-collection-member-previous">{f'<a href="/documents/{escape(str(item.get("previous_document_id") or ""))}">Previous in collection</a>' if item.get('previous_document_id') else 'Beginning of collection sequence'}</td>
          <td class="public-collection-member-next">{f'<a href="/documents/{escape(str(item.get("next_document_id") or ""))}">Next in collection</a>' if item.get('next_document_id') else 'End of collection sequence'}</td>
        </tr>"""
        for item in memberships
    )
    if not rows:
        rows = '<tr><td colspan="8" class="public-collection-empty">No active governed member documents are currently publicly visible for this collection.</td></tr>'
    return f"""<section class="public-collection-members"><h2>Governed Member Documents</h2><p class="public-collection-boundary">This collection presents governed memberships in declared sequence order. Sequence records navigational continuity only. It does not alter document identity, provenance, lifecycle, evidential status, factual meaning, authorship, legal status, or external validation.</p><div class="public-collection-members-wrapper"><table class="public-collection-members-table"><thead><tr><th class="public-collection-member-sequence">Public position</th><th class="public-collection-member-reference">Membership reference</th><th class="public-collection-member-document">Member document</th><th class="public-collection-member-document-reference">Document reference</th><th class="public-collection-member-format">Format</th><th class="public-collection-member-status">Publication status</th><th class="public-collection-member-previous">Previous</th><th class="public-collection-member-next">Next</th></tr></thead><tbody>{rows}</tbody></table></div></section>"""
