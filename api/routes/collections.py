    ...
    for item in rows:
        ...
        rendered.append(
            f"""<tr>
              ...
              <td class="public-collection-member-count">{int(item.get('visible_member_count') or 0)}</td>
...
def _render_collection_members(memberships: list[dict]) -> str:
    ...
    rows = "".join(
        f"""<tr>
          <td class="public-collection-member-sequence">Member {escape(_display(item.get('sequence_position')))} of {escape(_display(item.get('sequence_total')))}</td>
          <td class="public-collection-member-reference">{escape(_display(item.get('membership_reference')))}</td>
          <td class="public-collection-member-type">{member_badge(item)}</td>
          <td class="public-collection-member-document"><a href="{escape(str(item.get('member_url') or '#'))}">{escape(_display(item.get('member_title')))}</a>{f'<p>{escape(str(item.get("member_summary") or ""))}</p>' if item.get('member_summary') else ''}</td>
          <td class="public-collection-member-document-reference">{escape(_display(item.get('member_public_reference')))}</td>
          <td class="public-collection-member-format">{escape(_display(item.get('member_format')))}</td>
          <td class="public-collection-member-status">{escape(_display(item.get('member_status_label')))}</td>
          <td class="public-collection-member-section">{escape(_display(item.get('section_label')))}</td>
          <td class="public-collection-member-previous">{f'<a href="{escape(str(item.get("previous_member_url") or "#"))}">Previous in collection</a>' if item.get('previous_member_url') else 'Beginning of collection sequence'}</td>
          <td class="public-collection-member-next">{f'<a href="{escape(str(item.get("next_member_url") or "#"))}">Next in collection</a>' if item.get('next_member_url') else 'End of collection sequence'}</td>
        </tr>"""
...
def public_collection_page(collection_reference: str, return_to: str | None = None):
    ...
    return HTMLResponse(content=f"""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>{escape(_display(collection.get('title')))} | Public Archive Collection</title><link rel="canonical" href="{canonical}"><meta name="description" content="Public archive collection {escape(reference)}: {escape(_display(collection.get('title')))}."><style>*{{box-sizing:border-box}}body{{margin:0;background:#f7f7f4;color:#1f2933;font-family:system-ui,sans-serif}}main{{width:min(1120px,calc(100% - 32px));margin:32px auto 64px}}h1,h2{{color:#143a52}}a{{color:#245d61}}{PUBLIC_NAVIGATION_CSS}.public-collection-reference{{font:700 .9rem ui-monospace,monospace;letter-spacing:.03em;color:#555}}.public-collection-boundary{{padding:14px;border-left:4px solid #2e8b9a;background:#fff;line-height:1.55}}table{{width:100%;border-collapse:collapse;background:#fff}}th,td{{padding:10px;border:1px solid #e1dfd8;text-align:left;vertical-align:top;overflow-wrap:anywhere}}th{{width:230px;background:#faf9f5;color:#555}}.public-collection-members-wrapper,.public-collection-pathway-wrapper{{overflow-x:auto}}.public-collection-members-table{{min-width:1420px;table-layout:auto}}.public-collection-members-table th{{background:#143a52;color:#fff;width:auto}}.public-collection-member-sequence{{min-width:130px;white-space:nowrap}}.public-collection-member-reference{{min-width:220px;font-family:ui-monospace,monospace}}.public-collection-member-type{{min-width:180px;white-space:nowrap}}.public-collection-member-document{{min-width:260px}}.public-collection-member-document p{{margin:.35rem 0 0;color:#555;line-height:1.45}}.public-collection-member-document-reference{{min-width:180px}}.public-collection-member-format,.public-collection-member-status,.public-collection-member-section{{min-width:140px;white-space:nowrap}}.public-collection-member-previous,.public-collection-member-next{{min-width:190px}}.public-collection-pathway-table{{min-width:820px;table-layout:auto}}.public-collection-pathway-timestamp{{min-width:180px;white-space:nowrap}}.public-collection-pathway-action,.public-collection-pathway-actor{{min-width:150px}}.public-collection-pathway-state{{min-width:260px;width:100%}}.public-collection-empty{{padding:14px;background:#fff;border:1px solid #e1dfd8}}</style></head><body><main class="public-collection-detail">{public_primary_navigation(active="collections")}{public_breadcrumbs([("Home", "/"), ("Archive", archive_return), ("Collections", "/archive?type=public_collection"), (_display(collection.get("title")), None)])}{archive_back_link(archive_return)}<p>{object_type_badge("public_collection")}</p><p class="public-collection-reference">{escape(reference)}</p><h1>{escape(_display(collection.get('title')))}</h1><section class="public-collection-summary"><h2>Collection Summary</h2><table>{rows}</table></section><section><h2>Collection Governance Boundary</h2><p class="public-collection-boundary">{escape(BOUNDARY_TEXT)} Removing an object from a collection does not delete or invalidate that object, and object-level changes do not silently rewrite collection membership history.</p></section>{members_html}{pathway_html}</main></body></html>""")
