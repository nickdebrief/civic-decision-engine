...
def _render_collection_members(memberships: list[dict]) -> str:
    rows = "".join(
        f"""<tr>
          <td class="public-collection-member-sequence">{escape(_display(item.get('display_sequence')))}</td>
    ...
    if not rows:
        rows = '<tr><td colspan="6" class="public-collection-empty">No active governed member documents are currently publicly visible for this collection.</td></tr>'
    return f"""<section class="public-collection-members"><h2>Governed Member Documents</h2><p class="public-collection-boundary">This collection presents governed memberships that reference independently preserved public documents. It does not duplicate document content, change document lifecycle, or alter document provenance.</p><div class="public-collection-members-wrapper"><table class="public-collection-members-table"><thead><tr><th class="public-collection-member-sequence">Sequence</th><th class="public-collection-member-reference">Membership reference</th><th class="public-collection-member-document">Member document</th><th class="public-collection-member-document-reference">Document reference</th><th class="public-collection-member-format">Format</th><th class="public-collection-member-status">Publication status</th></tr></thead><tbody>{rows}</tbody></table></div></section>"""
...
def public_collection_page(collection_reference: str):
    ...
    try:
        ...
        memberships = acm.list_public_collection_memberships(conn, collection["id"], root=intake_root())
