...
class AdminSessionTests(unittest.TestCase):
    ...
    def test_stage28_audit_package_is_deterministic_and_complete(self):
        current_values = {
            definition[1]: f"Visible value for {definition[2]}"
            for definition in self.admin_session.STAGE22_NODE_DEFINITIONS
        }
        ...
        self.assertEqual(
            "Administrative Audit Package Available With Limitations",
            audit_package["audit_package_state"],
        )
        self.assertEqual(10, len(audit_package["audit_sections"]))
        required_fields = {
            "audit_section_id",
            "section_name",
            "section_category",
            "source_stage_or_output",
            "section_state",
            "visible_basis",
            "included_outputs",
            "preservation_basis",
        ...
        for section in audit_package["audit_sections"]:
            self.assertEqual(required_fields, set(section))
        section_names = {
            section["section_name"] for section in audit_package["audit_sections"]
        }
        for required_name in (
            "Record Identifier",
            "Current Administrative Outputs",
            "Evidence Readiness",
            "Dependency Mapping",
            "Pathway Stability",
            "Transition History",
            ...
            self.assertIn(required_name, section_names)
        summary = audit_package["audit_package_summary"]
        self.assertEqual(10, summary["total_audit_sections"])
        self.assertEqual(10, summary["included_sections"])
        self.assertEqual(0, summary["unavailable_sections"])
        self.assertEqual(30, summary["dependency_node_count"])
        self.assertEqual(8, summary["pathway_count"])
        self.assertEqual(11, summary["transition_entry_count"])
        self.assertEqual(14, summary["provenance_entry_count"])
        ...
        self.assertEqual(
            audit_package,
            self.admin_session.build_administrative_audit_package(
                "CREF-STAGE28-001",
                current_values,
                dependency_map,
                stability,
    ...
    def test_stage28_audit_package_states_are_deterministic(self):
        self.assertEqual(
            "Administrative Audit Package Not Available",
            self.admin_session.build_administrative_audit_package()[
                "audit_package_state"
            ],
        )
        partial = self.admin_session.build_administrative_audit_package(
            "CREF-STAGE28-PARTIAL"
        )
        self.assertEqual(
            "Administrative Audit Package Partially Available",
            partial["audit_package_state"],
        )
        self.assertGreater(
            partial["audit_package_summary"]["unavailable_sections"], 0
        )

    def test_stage28_rendering_depth_matches_report_mode(self):
        current_values = {
            definition[1]: f"Visible value for {definition[2]}"
            for definition in self.admin_session.STAGE22_NODE_DEFINITIONS
        }
        ...
        executive = self.admin_session._render_administrative_audit_package(
            audit_package, report_mode="executive"
        )
        review = self.admin_session._render_administrative_audit_package(
            audit_package, report_mode="review"
        )
        full = self.admin_session._render_administrative_audit_package(
            audit_package, report_mode="full"
        )

        for rendered in (executive, review, full):
            self.assertIn("<h2>Administrative Audit Package</h2>", rendered)
            self.assertIn("<h3>Audit Package Overview</h3>", rendered)
            self.assertIn("<h3>Audit Package Limitations</h3>", rendered)
        self.assertIn("stage28-audit-executive", executive)
        self.assertNotIn("Audit Package Summary Table", executive)
        self.assertNotIn("Full Administrative Audit Package", executive)
        ...
        self.assertNotIn("Visible Basis</th>", review)
        self.assertIn("stage28-audit-full", full)
        self.assertIn("Full Administrative Audit Package", full)
        self.assertIn("Visible Basis</th>", full)
        self.assertIn("Included Outputs</th>", full)
    ...
    def test_stage31_framework_continuity_is_deterministic_and_complete(self):
        current_values = {
            definition[1]: f"Visible value for {definition[2]}"
            for definition in self.admin_session.STAGE22_NODE_DEFINITIONS
        }
        ...
        audit = self.admin_session.build_administrative_audit_package("CREF-STAGE31-001", current_values, dependency, stability, transitions, provenance, replay, integrity)
        ...
        original_inputs = copy.deepcopy((report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, current_values))
        ...
        self.assertEqual("Framework Continuity Preserved With Limitations", continuity["continuity_state"])
        self.assertEqual(23, len(continuity["continuity_checks"]))
        required_fields = {
            "continuity_check_id",
            "check_name",
            "continuity_category",
            "expected_continuity_state",
            "observed_continuity_state",
            "continuity_result",
            "continuity_basis",
            "affected_stage_or_output",
            "limitation_statement",
        }
        for check in continuity["continuity_checks"]:
            self.assertEqual(required_fields, set(check))
        summary = continuity["continuity_summary"]
        self.assertEqual(23, summary["total_continuity_checks"])
        self.assertEqual(19, summary["continuous_checks"])
        self.assertEqual(4, summary["continuous_with_limitation_checks"])
        self.assertEqual(0, summary["unavailable_checks"])
        self.assertEqual(0, summary["continuity_gap_checks"])
        self.assertEqual((30, 8, 11, 14, 15, 14, 10, 19, 23), (
            ...
            summary["reflexive_closure_check_count"],
        ))
        self.assertEqual(0, summary["integrity_gap_count"])
        self.assertEqual(0, summary["audit_unavailable_section_count"])
        self.assertEqual(0, summary["certification_non_conformance_count"])
        self.assertEqual(0, summary["reflexive_closure_gap_count"])
        check_names = {check["check_name"] for check in continuity["continuity_checks"]}
        for required_name in (
            "Full Inspection Default", "Report Mode Availability",
            "Dependency Node Continuity", "Pathway Continuity",
            "Transition Entry Continuity", "Provenance Entry Continuity",
            "Replay Step Continuity", "Integrity Check Continuity",
            "Integrity Gap Absence", "Audit Section Continuity",
            "Audit Section Availability", "Certification Check Continuity",
            "Certification Non-Conformance Absence",
            "Reflexive Closure Check Continuity", "Reflexive Closure Gap Absence",
            "Replayable Output Continuity", "Non-Replayable Output Absence",
            "Methodological Limitation Continuity",
            "Classification Boundary Continuity",
            "Record Mutation Boundary Continuity",
            "Database Write Boundary Continuity", "Public API Boundary Continuity",
        ):
            self.assertIn(required_name, check_names)
        self.assertEqual(continuity, self.admin_session.build_framework_continuity(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, current_values))
        self.assertEqual(original_inputs, (report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, current_values))

    def test_stage31_gap_and_partial_states_are_deterministic(self):
        ...
        gap_dependency = copy.deepcopy(dependency)
        gap_dependency["nodes"] = gap_dependency["nodes"][:-1]
        gap = self.admin_session.build_framework_continuity(report, gap_dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, current_values)
        self.assertEqual("Framework Continuity Gap Detected", gap["continuity_state"])
        partial = self.admin_session.build_framework_continuity()
        self.assertEqual("Framework Continuity Partially Available", partial["continuity_state"])
        self.assertGreater(partial["continuity_summary"]["unavailable_checks"], 0)

    def test_stage31_rendering_depth_matches_report_mode(self):
        current_values = {definition[1]: f"Visible value for {definition[2]}" for definition in self.admin_session.STAGE22_NODE_DEFINITIONS}
        report = self.admin_session.build_stage21_report_structure()
        dependency = self.admin_session.build_determination_dependency_map(current_values)
        stability = self.admin_session.build_pathway_stability_analysis(dependency)
        ...
        closure = self.admin_session.build_reflexive_closure(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, current_values)
        continuity = self.admin_session.build_framework_continuity(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, current_values)
        executive = self.admin_session._render_framework_continuity(continuity, report_mode="executive")
        review = self.admin_session._render_framework_continuity(continuity, report_mode="review")
        full = self.admin_session._render_framework_continuity(continuity, report_mode="full")
        for rendered in (executive, review, full):
            self.assertIn("<h2>Framework Continuity</h2>", rendered)
            self.assertIn("<h3>Framework Continuity Overview</h3>", rendered)
            self.assertIn("<h3>Framework Continuity Limitations</h3>", rendered)
        self.assertIn("stage31-continuity-executive", executive)
        self.assertNotIn("Framework Continuity Summary Table", executive)
        self.assertNotIn("Full Framework Continuity", executive)
        ...
        self.assertNotIn("Continuity Basis</th>", review)
        self.assertIn("stage31-continuity-full", full)
        self.assertIn("Full Framework Continuity", full)
        self.assertIn("Expected Continuity State</th>", full)
        self.assertIn("Observed Continuity State</th>", full)
        self.assertIn("Continuity Basis</th>", full)
        self.assertIn("Affected Stage or Output</th>", full)
        self.assertIn("Limitation Statement</th>", full)
    ...
    def test_stage33_framework_governance_is_deterministic_and_complete(self):
        current_values = {definition[1]: f"Visible value for {definition[2]}" for definition in self.admin_session.STAGE22_NODE_DEFINITIONS}
        report = self.admin_session.build_stage21_report_structure()
        dependency = self.admin_session.build_determination_dependency_map(current_values)
        stability = self.admin_session.build_pathway_stability_analysis(dependency)
        ...
        replay = self.admin_session.build_deterministic_replay(dependency, stability, transitions, provenance)
        integrity = self.admin_session.build_framework_integrity_verification(report, dependency, stability, transitions, provenance, replay, current_values)
        audit = self.admin_session.build_administrative_audit_package("CREF-STAGE33-001", current_values, dependency, stability, transitions, provenance, replay, integrity)
        certification = self.admin_session.build_methodological_conformance_certification(report, dependency, stability, transitions, provenance, replay, integrity, audit, current_values)
        closure = self.admin_session.build_reflexive_closure(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, current_values)
        continuity = self.admin_session.build_framework_continuity(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, current_values)
        register = self.admin_session.build_framework_change_register(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, continuity, current_values)
        original_inputs = copy.deepcopy((report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, continuity, register))
        statement = self.admin_session.build_framework_governance_statement(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, continuity, register)

        self.assertEqual("Framework Governance Statement Available With Limitations", statement["governance_state"])
        self.assertEqual(25, len(statement["governance_principles"]))
        required_fields = {
            "principle_id", "principle_name", "governance_category",
            "affected_stage_or_output", "declared_governance_state",
            "observed_governance_state", "governance_result",
            "governance_basis", "limitation_statement",
        }
        for principle in statement["governance_principles"]:
            self.assertEqual(required_fields, set(principle))
        summary = statement["governance_summary"]
        self.assertEqual((25, 21, 4, 0, 0), (
            summary["total_governance_principles"], summary["declared_principles"],
            summary["principles_with_limitation"], summary["unavailable_principles"],
            summary["governance_gap_count"],
        ))
        self.assertEqual((30, 8, 11, 14, 15, 14, 10, 19, 23, 23, 25), (
            summary["dependency_node_count"], summary["pathway_count"],
            summary["transition_entry_count"], summary["provenance_entry_count"],
            summary["replay_step_count"], summary["integrity_check_count"],
            summary["audit_section_count"], summary["certification_check_count"],
            summary["reflexive_closure_check_count"], summary["continuity_check_count"],
            summary["change_register_entry_count"],
        ))
        names = {principle["principle_name"] for principle in statement["governance_principles"]}
        for required_name in (
            "Framework Stewardship", "Methodological Independence",
            "Deterministic Operation", "Visible Limitations", "Non-Mutation Boundary",
            "Classification Boundary", "Evidence Boundary", "Dependency Preservation",
            "Replay Preservation", "Integrity Preservation", "Audit Preservation",
            "Certification Preservation", "Continuity Preservation",
            "Change Register Preservation", "Public Inspectability", "Open Documentation",
            "Version Transparency", "Boundary Preservation", "Non-Inference Principle",
            "Non-Authority Principle", "Governance Statement Visibility",
            "Governance Gap Absence", "Framework Persistence Declaration",
            "Methodology Governance Availability", "Governance Limitation Visibility",
        ):
            self.assertIn(required_name, names)
        self.assertEqual(statement, self.admin_session.build_framework_governance_statement(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, continuity, register))
        self.assertEqual(original_inputs, (report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, continuity, register))

    def test_stage33_gap_and_partial_states_are_deterministic(self):
        current_values = {definition[1]: f"Visible value for {definition[2]}" for definition in self.admin_session.STAGE22_NODE_DEFINITIONS}
        report = self.admin_session.build_stage21_report_structure()
        dependency = self.admin_session.build_determination_dependency_map(current_values)
        stability = self.admin_session.build_pathway_stability_analysis(dependency)
        ...
        gap_dependency = copy.deepcopy(dependency)
        gap_dependency["nodes"] = gap_dependency["nodes"][:-1]
        gap = self.admin_session.build_framework_governance_statement(report, gap_dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, continuity, register)
        self.assertEqual("Framework Governance Statement Gap Detected", gap["governance_state"])
        partial = self.admin_session.build_framework_governance_statement()
        self.assertEqual("Framework Governance Statement Partially Available", partial["governance_state"])
        self.assertGreater(partial["governance_summary"]["unavailable_principles"], 0)

    def test_stage33_rendering_depth_matches_report_mode(self):
        current_values = {definition[1]: f"Visible value for {definition[2]}" for definition in self.admin_session.STAGE22_NODE_DEFINITIONS}
        report = self.admin_session.build_stage21_report_structure()
        dependency = self.admin_session.build_determination_dependency_map(current_values)
        stability = self.admin_session.build_pathway_stability_analysis(dependency)
        ...
        register = self.admin_session.build_framework_change_register(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, continuity, current_values)
        statement = self.admin_session.build_framework_governance_statement(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, continuity, register)
        executive = self.admin_session._render_framework_governance_statement(statement, report_mode="executive")
        review = self.admin_session._render_framework_governance_statement(statement, report_mode="review")
        full = self.admin_session._render_framework_governance_statement(statement, report_mode="full")
        for rendered in (executive, review, full):
            self.assertIn("<h2>Framework Governance Statement</h2>", rendered)
            self.assertIn("<h3>Framework Governance Overview</h3>", rendered)
            self.assertIn("<h3>Framework Governance Limitations</h3>", rendered)
        self.assertIn("stage33-governance-executive", executive)
        self.assertNotIn("Governance Principles Summary Table", executive)
        self.assertNotIn("Full Governance Principles", executive)
        ...
        self.assertNotIn("Governance Basis</th>", review)
        self.assertIn("stage33-governance-full", full)
        self.assertIn("Full Governance Principles", full)
        self.assertIn("Declared Governance State</th>", full)
        self.assertIn("Observed Governance State</th>", full)
        self.assertIn("Governance Result</th>", full)
    ...
    def test_stage34_framework_version_lineage_is_deterministic_and_complete(self):
        ...
        self.assertEqual("CREF Stages 31–40", relationships["Phase III"]["stage_scope"])
        self.assertIn("through CREF Stage 34", relationships["Phase III"]["relationship"])
    ...
    def test_stage34_rendering_depth_matches_report_mode(self):
        ...
        for rendered in (executive, review, full):
            ...
            self.assertIn("CREF Stages 31–40", rendered)
            self.assertIn("through CREF Stage 34", rendered)
    ...
    def test_stage35_framework_lifecycle_review_is_deterministic_and_complete(self):
        current_values = {definition[1]: f"Visible value for {definition[2]}" for definition in self.admin_session.STAGE22_NODE_DEFINITIONS}
        report = self.admin_session.build_stage21_report_structure()
        dependency = self.admin_session.build_determination_dependency_map(current_values)
        stability = self.admin_session.build_pathway_stability_analysis(dependency)
        ...
        replay = self.admin_session.build_deterministic_replay(dependency, stability, transitions, provenance)
        integrity = self.admin_session.build_framework_integrity_verification(report, dependency, stability, transitions, provenance, replay, current_values)
        audit = self.admin_session.build_administrative_audit_package("CREF-STAGE35-001", current_values, dependency, stability, transitions, provenance, replay, integrity)
        certification = self.admin_session.build_methodological_conformance_certification(report, dependency, stability, transitions, provenance, replay, integrity, audit, current_values)
        closure = self.admin_session.build_reflexive_closure(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, current_values)
        continuity = self.admin_session.build_framework_continuity(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, current_values)
        register = self.admin_session.build_framework_change_register(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, continuity, current_values)
        governance = self.admin_session.build_framework_governance_statement(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, continuity, register)
        lineage = self.admin_session.build_framework_version_lineage(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, continuity, register, governance)
        original_inputs = copy.deepcopy((report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, continuity, register, governance, lineage))
        ...
        self.assertEqual("Framework Lifecycle Review Available With Limitations", lifecycle["lifecycle_state"])
        self.assertEqual(28, len(lifecycle["lifecycle_review_items"]))
        self.assertEqual(8, len(lifecycle["lifecycle_relationships"]))
        required_fields = {
            "lifecycle_item_id", "lifecycle_item_name", "lifecycle_category",
            "affected_stage_or_output", "declared_lifecycle_state",
            "observed_lifecycle_state", "lifecycle_review_result",
            "lifecycle_basis", "limitation_statement",
        }
        for item in lifecycle["lifecycle_review_items"]:
            self.assertEqual(required_fields, set(item))
        summary = lifecycle["lifecycle_summary"]
        self.assertEqual((28, 24, 4, 0, 0), (
            summary["total_lifecycle_review_items"], summary["reviewed_items"],
            summary["items_with_limitation"], summary["unavailable_items"],
            summary["lifecycle_gap_count"],
        ))
        self.assertEqual((35, 3, 6), (
        ...
        names = {item["lifecycle_item_name"] for item in lifecycle["lifecycle_review_items"]}
        for required_name in (
            "Methodology Lifecycle Origin", "Phase I Lifecycle Visibility",
            "Phase II Lifecycle Visibility", "Phase III Lifecycle Visibility",
            "Implemented Stage Progression", "Stage Succession Continuity",
            "Evaluation Methodology Completion", "Reflexive Inspection Closure",
            "Governance Phase Initiation", "Continuity Preservation",
            "Change Register Preservation", "Governance Statement Preservation",
            "Version Lineage Preservation", "Dependency Preservation",
            "Pathway Preservation", "Transition Preservation", "Provenance Preservation",
            "Replay Preservation", "Integrity Preservation", "Audit Package Preservation",
            "Certification Preservation", "Reflexive Closure Preservation",
            "Lifecycle Boundary Visibility", "Non-Inference of Future Stages",
            "Non-Mutation Lifecycle Boundary", "Public Inspectability",
            "Documentation Availability", "Lifecycle Review Limitation Visibility",
        ):
            self.assertIn(required_name, names)
        relationships = {item["phase"]: item for item in lifecycle["lifecycle_relationships"]}
        self.assertEqual("CREF Stages 31–40", relationships["Phase III"]["stage_scope"])
        self.assertIn("through CREF Stage 35", relationships["Phase III"]["relationship"])
        self.assertEqual("CREF Stages 31–35", relationships["Current Governance"]["stage_scope"])
        self.assertEqual("CREF Stages 36–40", relationships["Future Boundary"]["stage_scope"])
        self.assertEqual(lifecycle, self.admin_session.build_framework_lifecycle_review(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, continuity, register, governance, lineage))
        self.assertEqual(original_inputs, (report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, continuity, register, governance, lineage))

    def test_stage35_gap_and_partial_states_are_deterministic(self):
        current_values = {definition[1]: f"Visible value for {definition[2]}" for definition in self.admin_session.STAGE22_NODE_DEFINITIONS}
        report = self.admin_session.build_stage21_report_structure()
        dependency = self.admin_session.build_determination_dependency_map(current_values)
        stability = self.admin_session.build_pathway_stability_analysis(dependency)
        ...
        gap_dependency = copy.deepcopy(dependency)
        gap_dependency["nodes"] = gap_dependency["nodes"][:-1]
        gap = self.admin_session.build_framework_lifecycle_review(report, gap_dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, continuity, register, governance, lineage)
        self.assertEqual("Framework Lifecycle Review Gap Detected", gap["lifecycle_state"])
        partial = self.admin_session.build_framework_lifecycle_review()
        self.assertEqual("Framework Lifecycle Review Partially Available", partial["lifecycle_state"])
        self.assertGreater(partial["lifecycle_summary"]["unavailable_items"], 0)

    def test_stage35_rendering_depth_matches_report_mode(self):
        current_values = {definition[1]: f"Visible value for {definition[2]}" for definition in self.admin_session.STAGE22_NODE_DEFINITIONS}
        report = self.admin_session.build_stage21_report_structure()
        dependency = self.admin_session.build_determination_dependency_map(current_values)
        stability = self.admin_session.build_pathway_stability_analysis(dependency)
        ...
        lineage = self.admin_session.build_framework_version_lineage(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, continuity, register, governance)
        lifecycle = self.admin_session.build_framework_lifecycle_review(report, dependency, stability, transitions, provenance, replay, integrity, audit, certification, closure, continuity, register, governance, lineage)
        executive = self.admin_session._render_framework_lifecycle_review(lifecycle, report_mode="executive")
        review = self.admin_session._render_framework_lifecycle_review(lifecycle, report_mode="review")
        full = self.admin_session._render_framework_lifecycle_review(lifecycle, report_mode="full")
        for rendered in (executive, review, full):
            self.assertIn("<h2>Framework Lifecycle Review</h2>", rendered)
            self.assertIn("<h3>Framework Lifecycle Review Overview</h3>", rendered)
            self.assertIn("<h3>Lifecycle Relationships</h3>", rendered)
            self.assertIn("<h3>Framework Lifecycle Review Limitations</h3>", rendered)
            self.assertIn("CREF Stages 31–40", rendered)
            self.assertIn("through CREF Stage 35", rendered)
            self.assertIn("CREF Stages 36–40", rendered)
        self.assertIn("stage35-lifecycle-executive", executive)
        self.assertNotIn("Lifecycle Review Summary Table", executive)
        self.assertNotIn("Full Lifecycle Review Items", executive)
        ...
        self.assertNotIn("Lifecycle Basis</th>", review)
        self.assertIn("stage35-lifecycle-full", full)
        self.assertIn("Full Lifecycle Review Items", full)
        self.assertIn("Declared Lifecycle State</th>", full)
        self.assertIn("Observed Lifecycle State</th>", full)
        self.assertIn("Lifecycle Review Result</th>", full)
    ...
    def test_admin_attachment_listing_displays_all_attachment_states(self):
        ...
        self.assertIn(">v13.0</text>", content)
