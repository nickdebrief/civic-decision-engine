...
CANDIDATE_GOVERNED_MODULES = frozenset({
    "tests.test_canonical_public_origin", "tests.test_run_governed_test_suite"
})
...
def governed_entries(root: Path, *, tracked: set[str] | None = None, untracked: set[str] | None = None) -> tuple[tuple[Entry, ...], tuple[str, ...]]:
    ...
    expected = set(GOVERNED_TRACKED_MODULES) | set(CANDIDATE_GOVERNED_MODULES)
    if tracked != set(GOVERNED_TRACKED_MODULES):
        raise ManifestError("manifest_tracked_inventory_mismatch")
    unknown = untracked - set(CANDIDATE_GOVERNED_MODULES) - set(EXCLUDED_UNTRACKED_MODULES)
    if unknown:
        raise ManifestError("manifest_unclassified_untracked_test")
    modules = expected - (set(CANDIDATE_GOVERNED_MODULES) - untracked)
    if set(CANDIDATE_GOVERNED_MODULES) - untracked:
        raise ManifestError("manifest_candidate_missing")
