...
The MBOX archive is the governed object. Contained messages are bounded projections of that governed object. Stage 36 does not automatically create Document Intake records, canonical records, associations, collection memberships, transmissions, or independent SHA-256 identities for contained messages or attachments.
...
Stage 36 identifies exact-byte duplicate candidates by contained-message digest. It does not remove, collapse, reorder, or merge duplicate messages. Duplicate indicators are discovery metadata only and do not alter governance identity.
