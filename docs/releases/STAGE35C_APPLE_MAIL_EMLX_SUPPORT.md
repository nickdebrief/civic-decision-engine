...
Focused tests cover valid `.emlx` intake, plain text messages, HTML-only messages, multipart messages, attachment metadata, XML and binary plist metadata, Apple flags extraction, original SHA-256 preservation, exact original `.emlx` download, public/private visibility, search, archive Email filtering, preview labels, validation failures, HTML sanitisation, and CDE Platform Stage 35A and CDE Platform Stage 35B regression compatibility.

## Intentional Exclusions

CDE Platform Stage 35C does not implement MBOX, PST, OST, Gmail Takeout, IMAP acquisition, mailbox-level import, email sending, independent attachment download, automatic attachment ingestion, automatic transmission creation, automatic association creation, or automatic collection membership.
